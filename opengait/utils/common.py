...
def params_count(net):
    ...
    tensor = torch.randn(1, 1, 1, 64, 44)
    # tensor = torch.rand(1, 1, 64, 44).cuda()
    # tensor = ([[tensor], None,None,None,None])

    # 分析FLOPs
    #flops = FlopCountAnalysis(net.Backbone.cuda(), tensor.cuda())

    #flops = FlopCountAnalysis(net.cuda(), tensor)
    #macs, params = profile(net.Backbone.cuda(), inputs=(tensor.cuda(),))
    #return 'Parameters Count: {:.5f}M MACs Count: {:.5f}G'.format(n_parameters / 1e6, macs/1e9)
    return 'Parameters Count: {:.5f}M MACs Count: {:.5f}G'.format(n_parameters / 1e6, 0)
