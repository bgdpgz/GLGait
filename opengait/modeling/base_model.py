...
class BaseModel(MetaModel, nn.Module):
    ...
    def get_optimizer(self, optimizer_cfg):
        ...
        #     if p.size() == torch.tensor([[[0,1,0]]]).size():
        #         param_conv1.append(p)
        # print(len(param_conv1))
        # param_groups = [
        #     {'params': filter(lambda p: p.requires_grad and p not in param_groups, self.parameters()), 'lr': 0.1},  # 第一个线性层的参数使用学习率0.1
        #     {'params': filter(lambda p: p in param_groups, self.parameters()), 'lr': 1e-7},  # 第二个线性层的参数使用学习率0.01
        # ]

        optimizer = optimizer(
            filter(lambda p: p.requires_grad, self.parameters()), **valid_arg)
        #optimizer = optimizer(param_groups, **valid_arg)
    ...
    def _load_ckpt(self, save_name):
        ...
        for i in model_state_dict:
            if i != "loss_aggregator.losses.triplet_fq.module.fc.weight" and i!="loss_aggregator.losses.triplet_fq.module.fc.bias":
                model_state_dict_1[i] = model_state_dict[i]
        ...
        self.load_state_dict(model_state_dict_1, strict=load_ckpt_strict)
        if self.training:
            ...
            if not self.engine_cfg["scheduler_reset"] and 'scheduler' in checkpoint:
                ...
                # self.scheduler.milestones = Counter({80000:1, 120000:1, 150000:1})
                # print(self.scheduler.milestones)
            else:
                self.msg_mgr.log_warning(
    ...
    def inputs_pretreament(self, inputs):
        ...
        """
        seqs_batch, labs_batch, typs_batch, vies_batch, ids_batch, index_batch, seqL_batch = inputs
        ...
        if seqL is not None:
            ...
            ipts = seqs
        del seqs
        return ipts, labs, typs, vies, ids, index, seqL
