...
class CollateFn(object):
    ...
    def __call__(self, batch):
        ...
        seqs_batch, labs_batch, typs_batch, vies_batch, ids_batch = [], [], [], [], []

        for bt in batch:
            seqs_batch.append(bt[0])
            labs_batch.append(self.label_set.index(bt[1][0]))
            ids_batch.append(bt[1][0])
        ...
        def sample_frames(seqs):
            ...
            return [sampled_fras, indices]

        # f: feature_num
        # b: batch_size
        # p: batch_size_per_gpu
        # g: gpus_num
        fras_index_batch = [sample_frames(seqs) for seqs in seqs_batch]  # [b, f]
        fras_batch = [i[0] for i in fras_index_batch]
        index_batch = [i[1] for i in fras_index_batch]
        batch = [fras_batch, labs_batch, typs_batch, vies_batch, ids_batch, index_batch, None]
